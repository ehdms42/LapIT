...
const card: React.CSSProperties = {
  background: "#fff", borderRadius: 20, border: "1px solid #e5e8eb",
...
export function Quiz() {
  ...
  if (done) {
    ...
    const grade = pct >= 80 ? { label: "우수", color: "#166534", bg: "#f0fdf4", border: "#bbf7d0" }
      : pct >= 60 ? { label: "보통", color: "#92400e", bg: "#fffbeb", border: "#fde68a" }
      ...
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div style={{ ...card, overflow: "hidden" }}>
          ...
          <div style={{ padding: "36px 32px", textAlign: "center" }}>
            ...
            <h2 style={{ fontSize: 24, fontWeight: 800, color: "#191f28", marginBottom: 24 }}>결과 확인</h2>
            <div style={{ padding: "24px", borderRadius: 20, marginBottom: 20,
              background: grade.bg, border: `2px solid ${grade.border}` }}>
              <p style={{ fontSize: 48, fontWeight: 900, color: grade.color, lineHeight: 1 }}>{pct}<span style={{ fontSize: 24 }}>점</span></p>
            ...
            <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: 24 }}>
              {Qs.map((_, i) => (
                <div key={i} style={{ width: 32, height: 32, borderRadius: 10, display: "flex",
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 800, color: "#191f28" }}>문제 풀이</h1>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={cur} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div style={{ padding: "20px 22px", borderBottom: "1px solid #f2f4f6" }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: "#191f28", lineHeight: 1.6 }}>{q.q}</p>
          </div>
            ...
            {q.opts.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => !shown && setSel(i)} style={{
                  ...
                  <span style={{ fontSize: 15, fontWeight: 800, color, flexShrink: 0, width: 20 }}>{LABELS[i]}</span>
                  <span style={{ fontSize: 13, fontWeight: 500, color, flex: 1, lineHeight: 1.5 }}>{opt}</span>
                  {shown && isCorr && <CheckCircle style={{ width: 16, height: 16, color: "#16a34a", flexShrink: 0 }} />}
                  {shown && !isCorr && isSel && <XCircle style={{ width: 16, height: 16, color: "#ef4444", flexShrink: 0 }} />}
                </button>
              );
            })}
          </div>
          ...
          <div style={{ padding: "0 22px 20px" }}>
            {!shown ? (
              <button onClick={submit} disabled={sel === null} style={{
                ...
                <CheckCircle style={{ width: 16, height: 16 }} /> 정답 확인
              </button>
            ) : (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div style={{ padding: "14px 16px", borderRadius: 12,
                  ...
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 6 }}>해설</p>
                ...
                <button onClick={next} style={{
                  ...
                  {cur < Qs.length - 1 ? <>다음 문제 <ArrowRight style={{ width: 16, height: 16 }} /></>
                    : <>결과 보기 <ArrowRight style={{ width: 16, height: 16 }} /></>}
                </button>
              </motion.div>
            )}
          </div>
      ...
      {/* 팁 */}
      <div style={{ padding: "14px 18px", borderRadius: 12, background: "#eff6ff",
        border: "1px solid #bfdbfe" }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: "#1d4ed8", marginBottom: 6 }}>학습 안내</p>
        <ul style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {["각 문제를 신중하게 읽고 핵심 개념을 파악하세요.",
            "틀린 문제는 해설을 통해 개념을 다시 확인하세요.",
            "이온·공유 결합 개념 페이지를 복습하면 도움이 됩니다."].map((t, i) => (
            <li key={i} style={{ fontSize: 12, color: "#1e40af", display: "flex", gap: 8 }}>
